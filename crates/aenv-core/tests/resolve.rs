...
fn resolves_two_level_chain_root_then_leaf() {
    let fs = MockFilesystem::new();
    write_manifest(&fs, "base", r#"
name = "base"
[adapters.claude-code]
files = ["CLAUDE.md"]
"#);
    write_file(&fs, "base", "CLAUDE.md", "# base\n");
    write_manifest(&fs, "leaf", r#"
name = "leaf"
extends = ["base"]
[adapters.claude-code]
files = ["CLAUDE.md"]
"#);
    ...
    let resolved = resolve_namespace(&fs, &registry(), &registry_with_cc(), &NamespaceId::new("leaf").unwrap()).unwrap();
    assert_eq!(resolved.chain, vec![NamespaceId::new("base").unwrap(), NamespaceId::new("leaf").unwrap()]);
...
fn detects_two_node_cycle() {
    let fs = MockFilesystem::new();
    write_manifest(&fs, "a", r#"
name = "a"
extends = ["b"]
"#);
    write_manifest(&fs, "b", r#"
name = "b"
extends = ["a"]
"#);
    let err = resolve_namespace(&fs, &registry(), &registry_with_cc(), &NamespaceId::new("a").unwrap()).unwrap_err();
    match err {
...
fn detects_self_cycle() {
    let fs = MockFilesystem::new();
    write_manifest(&fs, "selfish", r#"
name = "selfish"
extends = ["selfish"]
"#);
    let err = resolve_namespace(&fs, &registry(), &registry_with_cc(), &NamespaceId::new("selfish").unwrap()).unwrap_err();
...
fn resolves_diamond_without_duplication() {
    ...
    write_manifest(&fs, "left", r#"
name = "left"
extends = ["shared"]
"#);
    write_manifest(&fs, "right", r#"
name = "right"
extends = ["shared"]
"#);
    write_manifest(&fs, "top", r#"
name = "top"
extends = ["left", "right"]
"#);
    let resolved = resolve_namespace(&fs, &registry(), &registry_with_cc(), &NamespaceId::new("top").unwrap()).unwrap();
    let count_shared = resolved.chain.iter().filter(|n| n.as_str() == "shared").count();
    assert_eq!(count_shared, 1);
    assert_eq!(resolved.chain.iter().map(|n| n.as_str()).collect::<Vec<_>>(), vec!["shared", "left", "right", "top"]);
}

#[test]
fn rejects_unknown_namespace() {
    let fs = MockFilesystem::new();
    let err = resolve_namespace(&fs, &registry(), &registry_with_cc(), &NamespaceId::new("missing").unwrap()).unwrap_err();
...
fn rejects_manifest_name_directory_mismatch() {
    let fs = MockFilesystem::new();
    write_manifest(&fs, "alpha", r#"
name = "beta"
"#);
    let err = resolve_namespace(&fs, &registry(), &registry_with_cc(), &NamespaceId::new("alpha").unwrap()).unwrap_err();
...
fn rejects_reference_to_missing_adapter() {
    let fs = MockFilesystem::new();
    write_manifest(&fs, "ghost", r#"
name = "ghost"
[adapters.does-not-exist]
files = ["foo"]
"#);
    let err = resolve_namespace(&fs, &registry(), &registry_with_cc(), &NamespaceId::new("ghost").unwrap()).unwrap_err();
