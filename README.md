...
- **Manage skills.** `aenv skill new <name> --ns <ns>` scaffolds an authored skill whose files live in the namespace tree; `aenv skill import <source> --ns <ns>` pulls one in from a local path or git URL, with `--pin <ref>` for reproducibility and `--path <subdir>` for monorepo skill collections (k-dense-ai's `scientific-agent-skills`, etc.). `aenv skill import-all <source> --ns <ns>` bulk-imports every `<base>/<subdir>/SKILL.md` from a monorepo collection in a single command — `--only a,b` for a subset, `--base <dir>` to scope the scan — turning a 23-command onboarding into one (issue #1). `aenv skill remove <name> --ns <ns>` undoes either flavor; `aenv cache prune` reclaims `~/.aenv/cache/skills/` dirs nothing references.
- **Vendor non-skill content.** `aenv vendor <source> --ns <ns> --path <subtree> --as <dest>` copies agents, slash commands, reference docs — anything that isn't a skill — from a git source or local path into a namespace's tree, declares it under the owning adapter's `files`, and records provenance in a `[[vendored]]` table. Re-running refreshes from the source and reports per-file drift; `--pin <ref>` pins the source, `--adapter <a>` overrides the inferred owner, `--force` overwrites an unrelated collision. Hardened against path-traversal and symlink escape: `--as`/`--path` are validated before any write, source files that symlink outside the source tree are skipped with a warning, and a re-vendor never removes a file another entry still owns (issue #2).
