...
- **`aenv vendor <source> --ns <ns> --path <subtree> --as <dest> [--pin <ref>] [--adapter <a>] [--force]`** — copy non-skill content (agents, slash commands, reference docs, …) from a git source or local path into a namespace's tree, declare it under the owning adapter's `files`, and record provenance in a new `[[vendored]]` manifest table (issue #2). The owning adapter is inferred from the `--as` prefix (`.codex/…` → codex, else claude-code) unless `--adapter` is given. Re-running refreshes from the source and reports per-file drift (`+` changed / `=` unchanged); a collision with unrelated existing content errors unless `--force`. Vendored files are ordinary authored content under `files`, so activation and the resolver are unchanged. Added the `[[vendored]]` schema (`VendoredDecl`) and the shared `adapter_for_path` inference helper (now also used by snapshot/import). Hardened against path-traversal and symlink escape: `--as`/`--path` are rejected if they contain `..`/absolute components *before* any write, source files whose real location (via symlink) escapes the source tree are skipped with a warning, `--adapter` must name an installed adapter, and a re-vendor never removes a file another `[[vendored]]` entry still owns.
...
- **`--shared` flag emits `shared_files` at creation/capture time.** `aenv create <ns> --global --shared`, `aenv global new <ns> --shared`, `aenv global snapshot <ns> --shared`, and `aenv global import <src> --shared` now declare the scaffolded / captured / imported content as `shared_files` (both scopes from one copy) instead of `user_files` (global only) — so a dual-scope profile no longer needs a post-creation manifest edit. Opt-in; the default is unchanged (`user_files`). On `create`, `--shared` requires `--global` (a project namespace uses `files`); the auto-captured `baseline` is always global-only. Resolution now also emits a warning when a `files` entry and a `shared_files` remap target the same project path within one namespace+adapter (a silent double-manage). Completes the #5 follow-ups.
- **One stored copy across scopes — `shared_files`.** A namespace adapter block can now declare `shared_files = [".claude/CLAUDE.md", …]` (authored like `user_files`: user-scope layout, content stored once under the namespace's `user/` tree). At activation each entry materializes to `$HOME/<rel>` under `--global` and to the project under `--project` — from the *same* bytes, so reusing one `CLAUDE.md` per-project and globally no longer needs two hand-synced copies. The project destination is derived from the adapter's role map, so a role-tagged file lands in each scope's layout (e.g. `.claude/CLAUDE.md` → repo-root `CLAUDE.md` in a project, `~/.claude/CLAUDE.md` globally); non-role files keep the same relative path in both scopes. Editing the single source changes what both scopes materialize. Opt-in and backward compatible: existing `files` / `user_files` are unchanged. This is Layer 2 of #5. (See the `--shared` entry above for emitting `shared_files` at creation/capture time.)
