...
| Author a global profile from scratch | `aenv global new <name>` (scaffolds an editable `~/.aenv/envs/<name>/user/.claude/CLAUDE.md` + manifest), then `aenv global use <name>`. Add `--shared` to scaffold it as dual-scope (`shared_files`) from the start. |
...
| Add an arbitrary file to a global profile (settings.json, RTK.md, statusline/, …) | No CLI command — edit by hand: place the content at `~/.aenv/envs/<name>/user/<rel-path>` (mirroring its `$HOME` target, e.g. `user/.claude/RTK.md`), then add `<rel-path>` to the manifest's `[adapters.<adapter>] user_files` (a trailing `/` declares a whole directory), then `aenv global use <name>`. `user_files` is not capped by the adapter. |
| Capture the current `~/.claude/` as a reusable namespace | `aenv global snapshot <name> [--include <path>…] [--shared]` (`--shared` declares the captured content as `shared_files` so it serves project scope too) |
| Import a source without activating it | `aenv global import <source> [<name>] [--pin <ref>] [--shared]` (`--shared` → dual-scope `shared_files`) |
...
**One copy serving both scopes (`shared_files`):** to reuse one profile's content both globally and inside a project without storing it twice, declare those paths under `shared_files` in the adapter block instead of `user_files` — e.g. `[adapters.claude-code] shared_files = [".claude/CLAUDE.md", ".claude/agents/"]`. Content is stored once under `~/.aenv/envs/<ns>/user/`; `aenv activate <ns> --global` materializes it into `$HOME`, and `aenv use <ns>` + `aenv activate` (project) materializes the *same* bytes into the repo, with role-tagged files remapped to each scope's layout (`.claude/CLAUDE.md` → repo-root `CLAUDE.md` in a project). Emit `shared_files` directly at creation/capture time with the opt-in `--shared` flag on `aenv create <ns> --global --shared`, `aenv global new --shared`, `aenv global snapshot --shared`, or `aenv global import --shared` (default is unchanged — `user_files`; on `create`, `--shared` requires `--global`). Or migrate an existing global-only profile by renaming its `user_files` key to `shared_files` (no files move). `files`/`user_files` still mean project-only / user-only. Heads-up: declaring the same project destination via both `files` and a `shared_files` remap makes `aenv activate` print a collision warning — keep one declaration per path.
